...
**FIXED 2026-07-24:** all CacheManager writes now go through `db.update_cache_page`, a single-transaction read-merge-write (`BEGIN IMMEDIATE` holds the write lock across the read), so concurrent writers to the same URL can no longer erase each other's fields. New cache mutations must use `update_cache_page` (updates / drop / mutate kwargs), never load→modify→save over two connections.
