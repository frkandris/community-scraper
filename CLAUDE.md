...
**Scheduler**: the cost-saver twin schedule is ON (`schedule.saver_enabled: true`): a `search_only` collector (01:00→16:20 UTC, DataForSEO standard high-priority queue, zero LLM) and an `ai_only` extractor (16:35→00:20 UTC, inside DeepSeek's off-peak discount window). Runs are boxed by `run_pipeline(stop_at=…)` and stop gracefully at the window end; unfinished pairs carry over. The legacy combined cron (`cron_enabled`) is off. `ai_only` loads raw pages one pair at a time; never restore the old whole-cache materialization because it can OOM at production scale. A third managed job, **description enrichment** (`schedule.enrich_enabled`, `_enrich_run` in `main.py`), also runs only in the off-peak window (`enrich_cron` 16:30 → `enrich_until` 00:30 UTC) generating `short_description`/`long_description` (see `scraper/enrich.py`); it is bounded, idempotent/resumable (the `long_description` marker), self-gated to the off-peak window, resumes on startup if a restart lands mid-window, and deliberately does NOT reserve the run coordinator (coexists with `ai_only`; `_merge_source_urls` keeps enriched fields safe).
